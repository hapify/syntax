...
const HasNonAlphaNumeric = (s) => !/^[a-zA-Z0-9]+$/.test(s);
/** Convert replacement search for regexp */
const ForRegExp = (r) => r.search
...
class ConditionalPattern extends base_1.BasePattern {
    ...
    variable(variable) {
        if (['M', 'Models', 'Model'].includes(variable))
            return 'root';
        else if (['F', 'Fields'].includes(variable))
            return 'root.fields.list';
        else if (['D', 'Dependencies'].includes(variable))
            return 'root.dependencies';
        else if (['R', 'ReferencedIn', 'RefModels'].includes(variable))
            return 'root.referencedIn';
        else if (['P', 'PrimaryField'].includes(variable))
            return 'root.fields.primary';
        // Accesses
        else if (['A', 'Accesses'].includes(variable))
            return 'root.accesses.list';
        else if (['Ac', 'CreateAccess'].includes(variable))
            return 'root.accesses.create';
        else if (['Ar', 'ReadAccess'].includes(variable))
            return 'root.accesses.read';
        else if (['Au', 'UpdateAccess'].includes(variable))
            return 'root.accesses.update';
        else if (['Ad', 'RemoveAccess'].includes(variable))
            return 'root.accesses.remove';
        else if (['As', 'SearchAccess'].includes(variable))
            return 'root.accesses.search';
        else if (['An', 'CountAccess'].includes(variable))
