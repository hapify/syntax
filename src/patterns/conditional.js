  ...
  static execute(template, actions = []) {
    return template
      ...
      .replaceSyntaxPattern(
        actions,
        ElseIfPattern,
        (match, _count, _variable, _condition) => {
          // Get the full syntax
          const variable = ConditionalPattern._variable(_variable);
          const tester = ConditionalPattern._tester(_condition);
          const condition = ConditionalPattern._condition(
      ...
      .replaceSyntaxPattern(actions, ElsePattern, () =>
        ConditionalPattern._dynamic("} else {")
      )
      .replaceSyntaxPattern(actions, EndPattern, () =>
        ConditionalPattern._dynamic("}")
      );
  }
        ...
        .replace(Condition, match => {
          ...
          if (!replacement) {
            throw new InternalError(
              `[ConditionalPattern._condition] Cannot find condition replacement for match: ${match} (in :${trimed})`
            );
          }
